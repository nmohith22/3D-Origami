...
export function Paper({ mode, isSticky, showGrid, committedFolds, onCommitFold, onReplaceFold, ...props }) {
  ...
  const bind = useDrag(({ movement: [mx, my], active: dragActive, cancel, event, first }) => {
    ...
    if (first) {
      ...
      let editFold = null
      for (let i = committedFolds.length - 1; i >= 0; i--) {
        const fold = committedFolds[i]
        const toA = new THREE.Vector3(handle.p.x - fold.p1.x, handle.p.y - fold.p1.y, 0)
        // Edit this fold if the handle is on the folded side OR if it's a handle generated BY this fold!
        if (toA.dot(fold.normal) > 0.01 || handle.id.includes(`f${i}_`)) {
          editFold = fold
          break
        }
      }
      ...
      if (editFold) {
        dragState.current.editingFoldId = editFold.id
        const L = handle.p.distanceTo(new THREE.Vector2(editFold.p1.x, editFold.p1.y))
        const reconstructedB = new THREE.Vector2(
          handle.p.x - editFold.normal.x * 2 * L,
          handle.p.y - editFold.normal.y * 2 * L
        )
        dragState.current.B_max = reconstructedB
        
        const originalDistToCursor = L * (1 - Math.cos(editFold.angle))
        const dirAB = new THREE.Vector2(-editFold.normal.x, -editFold.normal.y).normalize()
        dragState.current.editOffset = dirAB.multiplyScalar(originalDistToCursor)
      } else {
        dragState.current.editingFoldId = null
        dragState.current.editOffset = new THREE.Vector2(0, 0)
      }
    }

    const scale = 0.015
    const H = PAPER_SIZE / 2
    const editOffsetX = dragState.current.editOffset ? dragState.current.editOffset.x : 0
    const editOffsetY = dragState.current.editOffset ? dragState.current.editOffset.y : 0
    
    const rawBx = Math.max(-H, Math.min(H, handle.p.x + mx * scale + editOffsetX))
    const rawBy = Math.max(-H, Math.min(H, handle.p.y - my * scale + editOffsetY))
    ...
    if (handle.type === 'edge') {
      ...
      // User requested corners should NEVER accidentally fold adjacent edges if they pull straight.
      // We enforce this by projecting their drag onto the diagonal vector pointing to the center (0,0)!
      const diagonalNormal = new THREE.Vector2(-handle.p.x, -handle.p.y).normalize()
      const rawDir = new THREE.Vector2(rawBx - handle.p.x, rawBy - handle.p.y)
      let projection = rawDir.dot(diagonalNormal)
      projection = Math.max(0, projection)
      const projectedX = handle.p.x + diagonalNormal.x * projection
      const projectedY = handle.p.y + diagonalNormal.y * projection
      currentProj.set(projectedX, projectedY)
    } else {
